...
#Last updated on July 8, 2019 by Dr. Jeff Chen
#This script is to convert 23andme SNP genotype files to *.bed, *.bim, *.fam file formats via plink.
	...
	if ($file=~m/23andme/){
		...
		print "$file\n"; ## Only pick up 23andme diles

		 system("cp /m/CPHG/KING/xc3m/openSNP/$file /m/CPHG/KING/xc3m/23andme_file_run/."); ## Move files to working directory

		 system("/home/xc3m/openSNP/plink  --23file $file -out $file");  ##C Call plink 1.9 version to convert the files to binary fomart
